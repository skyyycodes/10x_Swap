...
const DB_DIR = node_path_1.default.join(process.cwd(), 'data');
const DB_PATH = node_path_1.default.join(DB_DIR, 'db.sqlite');
...
try {
    ...
    if (!hasRules) {
        const rulesJsonPath = node_path_1.default.join(DB_DIR, 'rules.json');
    ...
    if (!hasLogs) {
        const logsJsonPath = node_path_1.default.join(DB_DIR, 'logs.json');
...
function getRules(ownerAddress) {
...
function updateRule(id, changes) {
    ...
    const merged = {
        ...existing,
        ...changes,
        targets: (changes.targets ?? existing.targets),
